...
public class ConnectionPool {
    ...
    public void init(String jdbcDriver, String jdbcUrl,
                     String jdbcUser, String jdbcPassword,
                     int minSize, int maxSize,
                     int validationTimeout) throws PoolException {
        try {
            if(minSize <= maxSize) {
    ...
    public void init(String jdbcDriver, String jdbcUrl, String jdbcUser,
                     ...
                     int maxSize) throws PoolException {
        init(jdbcDriver, jdbcUrl, jdbcUser, jdbcPassword, minSize, maxSize, 0);
    }

    public void init(String jdbcDriver, String jdbcUrl, String jdbcUser,
                     String jdbcPassword, int size) throws PoolException {
        init(jdbcDriver, jdbcUrl, jdbcUser, jdbcPassword, 0, size, 0);
    }

    public void init(String jdbcDriver, String jdbcUrl, String jdbcUser,
                     String jdbcPassword) throws PoolException {
        ...
        closer.execute(() -> {
            synchronized(connection) {
                try {
                    ...
                    logger.info("can not rollback in method close of class ConnectionPool");
                }
                try {
                    ((ConnectionWrapper)connection).getConnection().close();
                } catch(SQLException e) {
                    logger.info("can not close connection in method close of class ConnectionPool");
                }
            }
        });
    }
